...
...
```
We can use `tac` directly against the root flag to read it:
```
monitor@waldo:~/app-dev/v0.1$ tac /root/root.txt
8fb67c[REDACTED]4584f6c
```
But didn't the man page say it was cat in reverse? Does this mean that it also prints the file contents backwards? It is easy to test by tacing a file we know what to expect:
```
